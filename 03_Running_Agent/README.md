...
Ap agent ko Runner class ki madad se chalate hain. Aapke paas 3 options hoty hain Agent run karwany ky leye:
...
**Notes:**
- **Asynchronous** (async) methods mein await lagate hain, jisse app dusra kaam bhi kar sakta hai jab tak agent ka response aa raha ho.
- **Synchronous** Synchronous methods mein app ko wait karna parta hai jab tak kam complete na ho jaye, tab tak koi aur kaam nahi kar sakty.
...
### 🔸 The agent Loop
jab ap `Runner` mein `run` method use karty ho, tw ap 2 rugument pass karty ho **starting agent** aur **input** input ya tw ek string hoga (jesy user message) ya phr item ki list jo OpenAI Responses API ke items hoty hain,
...
3. Agar hum `max_turns` set karty hain aur wo exceed ho jata hai, to MaxTurnsExceeded exception milta hai.

**Notes:** SDK ka rule ye hay Jab LLM final_output generate kar leta hay tw wo desire type (yani wo job hum chahty hain jesy plain text) ka text output produce karta hay aur us mein koi tools calls nh hoty


### 🔸 Streaming
Streaming apko ye bhi allow karte hay ap LLM run ky time streaming events receive kar sakty hain. jesy hi stream complete hote hay, `RunResultStreaming` run ki sari information hoti hay, jis mein sary naye outputs hoty hain, Ap `.stream_events()` call karky streaming events ly sakty hain, Streaming matlab choty choty chunks mein output lena.
...
- **`handoff_input_filter`:** Ek global input filter hay jo sary agent per appply hota hay, Lekin sirf tab, jab handoff ke pass phely sy koi filter na ho. Ye input filter apko input edit karny ki ijazat deta hay mean keh koi aese specific word ya language dusry agent tak na pochy jo ap nh chahty.
...
**`workflow_name, trace_id, group_id`:** ye tracing ky leye important identifiers hain jo run ko uniqueky track karny aur organize karny mein help karta hay.
...
- **group_id:** Ye optional hota hay aur multiple runs ke traces ko ek group mein link karny ky leye hota hay. Matlab agar apky pass kayi runs hain jo related hain tw unhy group_id sy ek sath track kar sakty hain.
...
Agent run ka matlab hai ek poora round jisme agent apka sawal leta hai, kam karta hai, phr jawab deta hai. End mein ap decide karte ho ke user ko saare steps ka result dikhana hai ya sirf final output. Agar user dobara sawal kare to naya round start hota hai, aur ap phir run method call sakty ho.
...
**Note:** Agents SDK long-term use ky leye nh hay, agr long running use karna hay tw temporal use karna hoga.
...
- Malformed JSON: jab model tool calls ky leye apna directly output malformed JSON structure deta hay, khas tor per agar koi specific output define ho. Mean, jab LLM apny ouput ya tool karty waqt JSON data bhjta hay, Wo bilkul sahi aur complete hona chahiye kuch missing hoga tw `ModelBehaviorError` ayega
...
**`InputGuardrailTripwireTriggered, OutputGuardrailTripwireTriggered`:** Ye exception tab leta hay jab input guardrial ya output guardrial ke conditions puri hoti hain, input guardrial any waly message ko precess karny sy phely check karta hay, aur output guardrials agent ky final response sy phely check karta hay.
